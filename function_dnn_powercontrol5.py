...
def ini_weights(n_input, n_hidden_1, n_hidden_2, n_hidden_3, n_hidden_4,n_hidden_5,n_output):
    weights = {
        ...
        'h5': tf.Variable(tf.truncated_normal([n_hidden_4, n_hidden_5]) / np.sqrt(n_hidden_4)),
        'out': tf.Variable(tf.truncated_normal([n_hidden_5, n_output])) / n_hidden_5,
    }
    biases = {
        ...
        'b5': tf.Variable(tf.ones([n_hidden_5]) * 0.1),
...
def multilayer_perceptron(x, weights, biases,input_keep_prob,hidden_keep_prob):
    ...
    layer_4 = tf.nn.dropout(layer_4, hidden_keep_prob)
    
    layer_5 = tf.add(tf.matmul(layer_4, weights['h5']), biases['b5'])
    layer_5 = tf.nn.relu(layer_5)
    layer_5 = tf.nn.dropout(layer_5, hidden_keep_prob)
...
def train(X, Y,location, training_epochs=300, batch_size=1000, LR= 0.001, 
          n_hidden_1=200, n_hidden_2=200, n_hidden_3=200,n_hidden_4=200,n_hidden_5=200, 
          traintestsplit = 0.01, LRdecay=0):
    ...
    weights, biases = ini_weights(n_input, n_hidden_1, n_hidden_2, n_hidden_3, n_hidden_4,n_hidden_5,n_output)
...
def test(X, model_location, save_name, n_input, n_output, 
         n_hidden_1=200, n_hidden_2=200, n_hidden_3=200,n_hidden_4=200,n_hidden_5=200,  
         binary=0):
    ...
    if X.shape[0] < n_input:
        padding_width = n_input - X.shape[0]
        X = np.pad(X, ((0, padding_width), (0, 0)), 'constant')
